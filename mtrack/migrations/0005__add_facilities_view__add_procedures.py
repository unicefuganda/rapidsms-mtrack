...
class Migration(SchemaMigration):
    def forwards(self, orm):
        db.execute("DROP VIEW IF EXISTS facilities;")
        db.execute("DROP TRIGGER IF EXISTS update_facility_extras ON rapidsms_xforms_xformsubmission")
        db.execute("DROP TRIGGER IF EXISTS edit_healthfacility ON healthmodels_healthfacilitybase_catchment_areas")
        #create views and triggers
        run_file('0005__add_facilities_view__add_procedures.sql')
    def backwards(self, orm):
        db.execute("DROP VIEW facilities CASCADE;")
